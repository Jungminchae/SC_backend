...
def test_get_all_comment_with_login_should_pass(client):
...
def test_get_all_comment_without_login_should_pass(client):
    knowhow = mixer.blend(KnowHowPost)
    mixer.cycle(50).blend(KnowHowComment, post=knowhow)
...
def test_edit_comment_should_pass(client):
    # login
    User = get_user_model()
...
def test_delete_comment_should_pass(client):
    # login
    User = get_user_model()
    user_1 = mixer.blend(User, username=None)
    client.force_login(user_1)
...
def test_edit_reply_should_pass(client):
    # login
    User = get_user_model()
    user_1 = mixer.blend(User, username=None)
    client.force_login(user_1)
...
def test_delete_reply_should_pass(client):
    # login
    User = get_user_model()
    user_1 = mixer.blend(User, username=None)
    client.force_login(user_1)
...
def test_do_like_and_unlike_should_pass(client):
    client = one_user_login(client)
